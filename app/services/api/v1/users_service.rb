module Api
    module V1
        class UsersService    
            def self.initialize(token = nil, email = nil)
                if token != nil
            ...
            def self.add_token_to_cookie(cookies)
            ...
            def self.check_current_user(id, token)
            ...
            def self.create_return_object()
               wanted_att = @user.as_json
               wanted_att.delete('token')
               wanted_att
            end

            def self.is_loged_in
                @user != nil
            end
        end
    end
